...
# Load the pickled model (svc.pkl)
svc = pickle.load(open('models/svc.pkl', 'rb'))

# Load datasets into pandas DataFrames
description_df = pd.read_csv(r"C:\Users\GAGAN\OneDrive\Desktop\SYMP_TECH_AI\symptom\data\description.csv")
precautions_df = pd.read_csv(r"C:\Users\GAGAN\OneDrive\Desktop\SYMP_TECH_AI\symptom\data\precautions_df.csv")
medications_df = pd.read_csv(r"C:\Users\GAGAN\OneDrive\Desktop\SYMP_TECH_AI\symptom\data\medications.csv")
diets_df = pd.read_csv(r"C:\Users\GAGAN\OneDrive\Desktop\SYMP_TECH_AI\symptom\data\diets.csv")
workout_df = pd.read_csv(r"C:\Users\GAGAN\OneDrive\Desktop\SYMP_TECH_AI\symptom\data\workout_df.csv")
...
symptoms_dict = {'itching': 0, 'skin_rash': 1, 'nodal_skin_eruptions': 2, 'continuous_sneezing': 3, 'shivering': 4, 
                 ...
                 'inflammatory_nails': 128, 'blister': 129, 'red_sore_around_nose': 130, 'yellow_crust_ooze': 131 }  # Use your existing dictionary here
diseases_list = {15: 'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 'Drug Reaction', 33: 'Peptic ulcer disease', 
                 ...
                 31: 'Osteoarthritis', 5: 'Arthritis', 0: '(vertigo) Paroxysmal Positional Vertigo', 2: 'Acne', 38: 'Urinary tract infection',
                 35: 'Psoriasis', 27: 'Impetigo'}  # Use your existing dictionary here
...
def fetch_disease_info(disease_name):
    # Get description
    description_row = description_df[description_df['Disease'].str.lower() == disease_name.lower()]
    description = description_row['Description'].values[0] if not description_row.empty else "No description available."

    # Get precautions
    precautions_row = precautions_df[precautions_df['Disease'].str.lower() == disease_name.lower()]
    precautions = []
    ...
    # Get medications
    medications_row = medications_df[medications_df['Disease'].str.lower() == disease_name.lower()]
    medications = medications_row['Medication'].tolist() if not medications_row.empty else []

    # Get diets
    diets_row = diets_df[diets_df['Disease'].str.lower() == disease_name.lower()]
    diets = diets_row['Diet'].tolist() if not diets_row.empty else []

    # Get workouts
    workout_row = workout_df[workout_df['disease'].str.lower() == disease_name.lower()]
    workouts = workout_row['workout'].tolist() if not workout_row.empty else []
